...
void FireballGraphics::assemble(sf::VertexArray &canvas,
                                std::shared_ptr<Update> genericUpdate, sf::IntRect texCoords)
{
    std::shared_ptr<FireballUpdate> fu = std::static_pointer_cast<FireballUpdate>(genericUpdate);
    m_Position = fu->getPositionPointer();
    m_FacingRight = fu->getFacingRightPointer();

    m_Animator = new Animator(texCoords.position.x,
                              texCoords.position.y,
                              3, //  6帧
                              texCoords.size.x * 3, texCoords.size.y,
                              6); //  帧率
...
void FireballGraphics::draw(VertexArray &canvas)
{
    ...
    canvas[m_VertexStartIndex + 2].position = position + Vector2f(0, scale.y);

    canvas[m_VertexStartIndex + 3].position = position + Vector2f(scale.x, 0);
